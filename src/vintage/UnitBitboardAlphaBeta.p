            ...
            {$IFC FALSE}
             if (kVerbosityLevelAlgoParallele >= 3) and (nbreNoeudsGeneresFinale >= kNombreDeNoeudMinimalPourSuiviDansRapport) then
                begin
                  ...
                  WritelnNumDansRapport('avant l''appel à CalculerCoupsEnParallele par la thread '+IntToStr(nroThread)+', nroThreadEsclave = ',nroThreadEsclave);
          ...
          //  {$IFC AVEC_DEBUG_PARALLELISME}
           {$IFC FALSE}
             if (kVerbosityLevelAlgoParallele >= 3) and (nbreNoeudsGeneresFinale >= kNombreDeNoeudMinimalPourSuiviDansRapport) then
                begin
                  errDebug := MPEnterCriticalRegion(gRapportCriticalRegionID,kDurationForever);
                  WritelnNumDansRapport('apres l''appel à CalculerCoupsEnParallele par la thread '+IntToStr(nroThread)+',  nroThreadEsclave = ',nroThreadEsclave);
                  AttendreFrappeClavierParallelisme(true);
                  errDebug := MPExitCriticalRegion(gRapportCriticalRegionID);
                end;
            {$ENDC}
