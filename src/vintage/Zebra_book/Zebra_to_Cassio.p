              ...
              if not(CassioEstEnTrainDeLireLaBibliothequeDeZebra) then
                begin
                  ...
                  if ((TickCount - ticks) > 1) then
                    begin
                      (*
                      s := ReplaceParameters(ReadStringFromRessource(TextesDiversID,12),IntToStr(number_of_positions_in_zebra_book),'','','');  {s := 'Nb de positions dans la bibliothèque de Zebra : ^0'}
